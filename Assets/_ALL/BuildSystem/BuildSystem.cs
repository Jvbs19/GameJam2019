...
public class BuildSystem : MonoBehaviour
{
    ...
    private void RayDrag()//simple ray cast from the main camera. Notice there is no range
    {
        Ray ray = cam.ScreenPointToRay(Input.mousePosition);
        RaycastHit hit;

        if (Physics.Raycast(ray, out hit, layer))//Segundo raycast para a movimentacao dos objetos
        {
            if (hit.transform.tag == "Building")
            {
                if (Input.GetMouseButtonDown(0))
                {
                    GameObject Switch = hit.transform.gameObject.GetComponent<DragDrop>().Preview.gameObject;
                    if (hit.transform.gameObject.GetComponent<DragDrop>().OnGrid == true)
                    {
                        NewBuild(Switch);
                        GetComponent<BuildSelector>().TogglePanel();
